...
const TextEditor = ({ submitButton, setsubmitButton, open }) => {
    ...
    useEffect(() => {
        const fetchNotesData = async () => {
            try {
                const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/notes/getNotes`, { username, quesId });
    ...
    useEffect(() => {
        if (submitButton) {
            const storeNotesData = async () => {
                ...
                try {
                    await axios.post(`${import.meta.env.VITE_API_URL}/api/notes/storeNotes`, { username, quesId, notes });
